...
const Level4Visualization = ({ mynngiftConfig, userAddress }) => {
  ...
  return (
    <div className="noblegift-visualization-container bg-gradient-to-b from-[#1A3A6A] to-[#102E50] p-4 sm:p-8 rounded-xl shadow-2xl min-h-0 flex flex-col items-center justify-center w-full">
      ...
      <div className="w-full text-center mb-8">
        <h2 className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-[#4DA8DA] via-[#00FF88] to-[#4DA8DA] bg-clip-text text-transparent mb-2 drop-shadow-lg">💎 Level 4 - Viscount Journey</h2>
        <div className="h-1 w-24 bg-gradient-to-r from-[#4DA8DA] to-[#00FF88] rounded-full mx-auto"></div>
      </div>
          <div className="mt-6">
      ...
      <div className="relative w-full max-w-4xl mx-auto bg-gradient-to-b from-[#102E50] to-[#0A1E2E] rounded-xl border border-[#4DA8DA]/40 overflow-hidden flex flex-col items-center justify-center aspect-[3/6] min-h-0 shadow-2xl hover:border-[#4DA8DA]/60 transition-all duration-300">
        <svg className="w-full h-full" viewBox="0 0 1200 600" preserveAspectRatio="xMidYMid meet">
          ...
          {/* Single Rank 4 Visualization - Simplified */}
          {(() => {
            const rank = 4;
            ...
            const slotRadius = 35;
            const slotPositions = [
              { dx: 0, dy: -120 }, { dx: 100, dy: -60 }, { dx: 100, dy: 60 },
              { dx: 0, dy: 120 }, { dx: -100, dy: 60 }, { dx: -100, dy: -60 }
            ];

            return (
              <g key={`rank-${rank}`} transform={`translate(${x}, ${y})`}>
...
export default Level4Visualization;
